...
def bottlegraph(jobid = None, resultsdir = None, outputfile = './bottlegraph', partial = None, no_text = False, thread_names_translate = translateThreadNameJikes):
  ...
  for threadid, name in list(stats.get_thread_names().items()):
  ...
  threads = sorted(list(xs.keys()), key = lambda thread: xs[thread], reverse = True)
...
''' % (os.path.basename(outputfile), max_x, max_x, max_y, ','.join([ '"%d" %d' % (abs(x), x) for x in range(-max_x, max_x+1) ])), file=fd)
