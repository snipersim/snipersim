...
def parse_stats(resultsdir, (k1, k2), ncores):
  simstats = os.path.join(resultsdir, 'sim.stats')
  simstats = os.path.exists(simstats) and open(simstats) or None

  simstatsbase = os.path.join(resultsdir, 'sim.stats.base')
  simstatsbase = os.path.exists(simstatsbase) and open(simstatsbase) or None

  simstatsdelta = os.path.join(resultsdir, 'sim.stats.delta')
