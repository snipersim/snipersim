...
SchedulerStaticMask::SchedulerStaticMask(ThreadManager *thread_manager)
   : Scheduler(thread_manager)
{
   ...
   {
       m_core_mask[core_id] = Sim()->getCfg()->getBoolArray("scheduler/static_mask/core_mask", core_id);
   }
}

core_id_t SchedulerStaticMask::findFirstFreeMaskedCore()
{
   for (core_id_t core_id = 0; core_id < (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id++)
   {
...
core_id_t SchedulerStaticMask::threadCreate(thread_id_t thread_id)
{
