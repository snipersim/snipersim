...
GhbPrefetcher::GhbPrefetcher(String configName, core_id_t core_id)
    : m_prefetchWidth(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/ghb/width", core_id)), m_prefetchDepth(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/ghb/depth", core_id)), m_lastAddress(INVALID_ADDRESS), m_ghbSize(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/ghb/ghb_size", core_id)), m_ghbHead(0), m_generation(0), m_ghb(m_ghbSize), m_tableSize(Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/ghb/ghb_table_size", core_id)), m_tableHead(0), m_ghbTable(m_tableSize)
{
}
