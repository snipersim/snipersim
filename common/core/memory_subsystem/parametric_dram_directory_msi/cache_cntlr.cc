...
#  define VALIDATE(address, data_buf, data_length) LOCKED(/*LOGID(); printf("validation @ %lx..+%u: ", address, data_length); DUMPDATA(data_buf, data_length); printf("\n");*/ LOG_ASSERT_ERROR(Sim()->ValidateDataRead(address, data_buf, data_length), "Data corruption @ %lx", address); )
#  define VALIDATE_PUT(address, data_buf, data_length) Sim()->ValidateDataUpdate(address, data_buf, data_length)
#else
#  define MYLOG(...) {}
#  define VALIDATE(...) {}
#  define VALIDATE_PUT(...) {}
...
CacheCntlr::CacheCntlr(MemComponent::component_t mem_component,
   ...
   m_perfect_llc(Sim()->getCfg()->getBool("perf_model/perfect_llc", false)),
   m_core_id(core_id),
   ...
   m_shmem_perf_model(shmem_perf_model)
{
   ...
   UInt32 process_num_me = Sim()->getConfig()->getCurrentProcessNum(),
          process_num_master = Sim()->getConfig()->getProcessNumForCore(m_core_id_master);
   LOG_ASSERT_ERROR(process_num_me == process_num_master, "%s @ core #%u running on both process %u and %u. Shared caches should run in the same process. Reduce the number of processes, or fix the thread allocation.", name.c_str(), m_core_id, process_num_me, process_num_master);
...
LOG_ASSERT_ERROR(offset + data_length <= getCacheBlockSize(), "access until %u > %u", offset + data_length, getCacheBlockSize());
   ...
   if (modeled && m_enabled)
   {
      ...
      updateCounters(mem_op_type, ca_address, cache_hit, getCacheState(ca_address), false);
   }

   if (cache_hit)
   {
MYLOG("L1 hit");
      // One could check the MSHR here, but since memory ops are so much out of order, this is pointless
      // I.e., a previous miss would return +1000 cycles, but a subsequent access (hit) to the same cache line
      // would still have the old issue time. If we went by the MSHR, it would have a 1000-cycle latency as well.
...
MYLOG("processMemOpFromCore l%d before next fill", m_mem_component);
         hit_where = m_next_cache_cntlr->processShmemReqFromPrevCache(this, mem_op_type, ca_address, false, false, t_start, true);
MYLOG("processMemOpFromCore l%d after next fill", m_mem_component);
      ...
      copyDataFromNextLevel(mem_op_type, ca_address);
   ...
   if (mem_op_type == Core::WRITE) {
      VALIDATE_PUT(ca_address + offset, data_buf, data_length);
   }

   accessCache(mem_op_type, ca_address, offset, data_buf, data_length);
MYLOG("access done");

   VALIDATE(ca_address + offset, data_buf, data_length);
   ...
   if (! cache_hit && modeled) {
      ...
      #ifdef ENABLE_MSHR
      m_master->mshr[ca_address] = make_mshr(t_start, t_now);
      #endif
   }
...
{
   ScopedLock sl(getLock());
   getCache()->updateCounters(1);
   updateCounters(mem_op_type, 0, 1, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, false);
}


void
CacheCntlr::copyDataFromNextLevel(Core::mem_op_t mem_op_type, IntPtr address)
{
...
MYLOG("copyDataFromNextLevel l%d", m_mem_component);
   ...
   m_next_cache_cntlr->retrieveCacheBlock(address, data_buf);
VALIDATE(address, data_buf, m_next_cache_cntlr->getCacheBlockSize());
...
HitWhere::where_t
CacheCntlr::processShmemReqFromPrevCache(CacheCntlr* requester, Core::mem_op_t mem_op_type, IntPtr address, bool modeled, bool isPrefetch, SubsecondTime t_issue, bool have_write_lock)
{
   #ifdef PRIVATE_L2_OPTIMIZATION
   ...
   if (cache_hit)
   {
      ...
      if (mem_op_type != Core::READ) {
         ...
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
            if (*it != requester) {
               std::pair<SubsecondTime, bool> res = (*it)->updateCacheBlock(address, CacheState::INVALID, Transition::COHERENCY);
...
MYLOG("add latency %s, sibling_hit(%u)", itostr(latency).c_str(), sibling_hit);
         ...
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
            if (*it != requester) {
               std::pair<SubsecondTime, bool> res = (*it)->updateCacheBlock(address, CacheState::SHARED, Transition::COHERENCY);
               latency = getMax<SubsecondTime>(latency, res.first);
               sibling_hit |= res.second;
            }
         }
MYLOG("add latency %s, sibling_hit(%u)", itostr(latency).c_str(), sibling_hit);
      ...
      if (cache_block_info && (cache_block_info->getCState() == CacheState::SHARED)) {
         ...
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++)
            if (*it != requester)
               latency = getMax<SubsecondTime>(latency, (*it)->updateCacheBlock(address, CacheState::INVALID, Transition::UPGRADE).first);
         getMemoryManager()->incrElapsedTime(latency, ShmemPerfModel::_USER_THREAD);
         atomic_add_subsecondtime(stats.snoop_latency, latency);
         assert(! cache_block_info->hasCachedLoc());
      }
      ...
      if (m_next_cache_cntlr) {
         ...
         if (m_perfect_llc) {
            ...
            if (cache_block_info)
               ...
               insertCacheBlock(address, mem_op_type == Core::READ ? CacheState::SHARED : CacheState::MODIFIED, NULL, ShmemPerfModel::_USER_THREAD);
         } else {
            initiateDirectoryAccess(mem_op_type, address, isPrefetch, t_issue);
         }
      }
   }
...
MYLOG("writethrough start");
            m_next_cache_cntlr->writeCacheBlock(ca_address, offset, data_buf, data_length);
VALIDATE(ca_address + offset, data_buf, data_length);
...
{
   CacheState::cstate_t old_cstate = getCacheState(address);
...
void
CacheCntlr::retrieveCacheBlock(IntPtr address, Byte* data_buf)
{
   SharedCacheBlockInfo* cache_block_info = (SharedCacheBlockInfo*) m_master->m_cache->accessSingleLine(address, Cache::LOAD, data_buf, getCacheBlockSize());
...
MYLOG("evicting @%lx", evict_address);
      ...
      if (! m_master->m_prev_cache_cntlrs.empty()) {
         ...
         SubsecondTime latency = SubsecondTime::Zero();
         for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++)
            latency = getMax<SubsecondTime>(latency, (*it)->updateCacheBlock(evict_address, CacheState::INVALID, Transition::EVICT_LOWER).first);
      ...
      if (m_next_cache_cntlr) {
         if (m_cache_writethrough) {
            ...
            if (evict_block_info.getCState() == CacheState::MODIFIED)
               m_next_cache_cntlr->writeCacheBlock(evict_address, 0, evict_buf, getCacheBlockSize());
         }
         m_next_cache_cntlr->notifyPrevLevelEvict(m_core_id_master, m_mem_component, evict_address);

      } else if (! m_perfect_llc) {
...
VALIDATE(evict_address, evict_buf, getCacheBlockSize());
...
CacheCntlr::updateCacheBlock(IntPtr address, CacheState::cstate_t new_cstate, Transition::reason_t reason, Byte* out_buf)
{
   ...
   if (! m_master->m_prev_cache_cntlrs.empty())
      for(CacheCntlrList::iterator it = m_master->m_prev_cache_cntlrs.begin(); it != m_master->m_prev_cache_cntlrs.end(); it++) {
         std::pair<SubsecondTime, bool> res = (*it)->updateCacheBlock(address, new_cstate, reason == Transition::EVICT ? Transition::EVICT_LOWER : reason);
   ...
   if (!cache_block_info) {
      ...
      if (cache_block_info->getCState() == CacheState::MODIFIED) {
         ...
         if (m_cache_writethrough) {
            ...
            Byte data_buf[getCacheBlockSize()];
            retrieveCacheBlock(address, data_buf);
            m_next_cache_cntlr->writeCacheBlock(address, 0, data_buf, getCacheBlockSize());
            is_writeback = true;
            sibling_hit = true;

         } else if (out_buf) {
            /* someone (presumably the directory interfacing code) is waiting to consume the data */
            retrieveCacheBlock(address, out_buf);
VALIDATE(address, out_buf, getCacheBlockSize());
            buf_written = true;
            is_writeback = true;
            sibling_hit = true;
...
void
CacheCntlr::writeCacheBlock(IntPtr address, UInt32 offset, Byte* data_buf, UInt32 data_length)
{
MYLOG(" ");
...
VALIDATE(address + offset, data_buf, data_length);
   } else {
      __attribute(__unused__) SharedCacheBlockInfo* cache_block_info = (SharedCacheBlockInfo*) m_master->m_cache->accessSingleLine(address + offset, Cache::STORE, data_buf, data_length);
VALIDATE(address + offset, data_buf, data_length);
...
MYLOG("have SHARED, upgrading to MODIFIED for #%u", request->cache_cntlr->m_core_id);
         ...
         if (! request->isPrefetch) { // Prefetches don't wait around, just write the data up and be gone
...
MYLOG("processInvReqFromDramDirectory l%d", m_mem_component);
   ...
   if (cstate != CacheState::INVALID)
   {
      ...
      updateCacheBlock(address, CacheState::INVALID, Transition::COHERENCY);
...
MYLOG("processFlushReqFromDramDirectory l%d", m_mem_component);
   ...
   if (cstate != CacheState::INVALID)
   {
      ...
      // Flush the line
      Byte data_buf[getCacheBlockSize()];
      updateCacheBlock(address, CacheState::INVALID, Transition::COHERENCY, data_buf);
...
MYLOG("processWbReqFromDramDirectory l%d", m_mem_component);
   ...
   if (cstate != CacheState::INVALID)
   {
      ...
      // Write-Back the line
      Byte data_buf[getCacheBlockSize()];
      updateCacheBlock(address, CacheState::SHARED, Transition::COHERENCY, data_buf);
...
void
CacheCntlr::updateCounters(Core::mem_op_t mem_op_type, IntPtr address, bool cache_hit, CacheState::cstate_t state, bool isPrefetch)
{
   ...
   if (m_master->mshr.size() > 8) {
      IntPtr address_min = NULL;
