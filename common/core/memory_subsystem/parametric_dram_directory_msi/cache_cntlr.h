...
namespace ParametricDramDirectoryMSI
{
   ...
   public:
      ...
      CacheParameters()
          : data_access_time(NULL, 0), tags_access_time(NULL, 0), writeback_time(NULL, 0)
      {
      }
      CacheParameters(
   ...
   public:
      ...
      CacheDirectoryWaiter(bool _exclusive, CacheBlockInfo::block_type_t _block_type, bool _isPrefetch, CacheCntlr *_cache_cntlr, SubsecondTime _t_issue) : exclusive(_exclusive), block_type(_block_type), isPrefetch(_isPrefetch), cache_cntlr(_cache_cntlr), t_issue(_t_issue)
      {
      }
   };
   ...
   private:
      ...
      CacheMasterCntlr(String name, core_id_t core_id, UInt32 outstanding_misses)
          : m_cache(NULL), m_prefetcher(NULL), m_dram_cntlr(NULL), m_dram_outstanding_writebacks(NULL), m_l1_mshr(name + ".mshr", core_id, outstanding_misses), m_next_level_read_bandwidth(name + ".next_read", core_id), m_evicting_address(0), m_evicting_buf(NULL), m_atds(), m_prefetch_list(), m_prefetch_next(SubsecondTime::Zero())
      {
      }
      ~CacheMasterCntlr();
   ...
   private:
      ...
      struct
      {
         ...
         UInt64 prefetch_latency_not_captured; // Prefetches that were not captured by the cache, e.g. because they were issued after the cache was disabled
      ...
      std::unordered_map<IntPtr, SubsecondTime> m_prefetch_timing_map;
