...
CacheSetSRRIP::CacheSetSRRIP(
    ...
    : CacheSet(cache_type, associativity, blocksize, is_tlb_set),
      m_rrip_numbits(Sim()->getCfg()->getIntArray(cfgname + "/srrip/bits", core_id)), m_rrip_max((1 << m_rrip_numbits) - 1), m_rrip_insert(m_rrip_max - 1), m_num_attempts(num_attempts), m_replacement_pointer(0), m_set_info(set_info), m_srrip_tlb_enabled(Sim()->getCfg()->getBoolArray(cfgname + "/srrip/tlb_enabled", core_id))
{
...
void CacheSetSRRIP::updateReplacementIndex(UInt32 accessed_index)
{
