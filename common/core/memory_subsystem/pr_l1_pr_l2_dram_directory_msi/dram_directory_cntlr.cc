...
#define MYLOG(...)                                                                                                                                                                                                    \
   {                                                                                                                                                                                                                  \
      ScopedLock l(iolock);                                                                                                                                                                                           \
      fflush(stdout);                                                                                                                                                                                                 \
      printf("[%s] %d%cdd %-25s@%3u: ", itostr(getShmemPerfModel()->getElapsedTime()).c_str(), getMemoryManager()->getCore()->getId(), Sim()->getCoreManager()->amiUserThread() ? '^' : '_', __FUNCTION__, __LINE__); \
      printf(__VA_ARGS__);                                                                                                                                                                                            \
      printf("\n");                                                                                                                                                                                                   \
      fflush(stdout);                                                                                                                                                                                                 \
   }
...
namespace PrL1PrL2DramDirectoryMSI
{
   ...
   DramDirectoryCntlr::DramDirectoryCntlr(core_id_t core_id,
                                          ...
                                          ShmemPerfModel *shmem_perf_model) : m_memory_manager(memory_manager),
                                                    ...
                                                    core_id_t receiver, IntPtr address, Byte *cached_data_buf, ShmemMsg *orig_shmem_msg)
   {
      ...
      else
      {
         ...
         if (m_protocol == CoherencyProtocol::MESIF && shmem_req->getForwardingFrom() == INVALID_CORE_ID // If forwarding already failed once, don't try again
             && directory_entry->getDirectoryBlockInfo()->getDState() == DirectoryState::SHARED && directory_entry->getForwarder() != INVALID_CORE_ID)
         {
